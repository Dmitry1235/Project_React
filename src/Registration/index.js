  ...
  validConfirmPassword(value){
    value === this.state.password.value ?
          this.setState({password: {value: this.state.password.value, error: false},
                        confirmPassword: {value: value, error: false}}) :
          this.setState({password: {value: this.state.password.value, error: true},
                        confirmPassword: {value: value, error: true}});
  }
        ...
        <div className='row'>
          ...
          <div className='Registration col-4'>
            ...
            <div>
              ...
              <div>
                <input type='text' className={`${this.state.password.error === true ? 'Red' : ''}`} value={password.value} onChange={(e) => this.handleChangeInput(e.target.value, 'password')}/></div>
              <label>Confirm password</label>
              <div>
                <input type='text' className={`${this.state.confirmPassword.error === true ? 'Red' : ''}`} value={confirmPassword.value} onChange={(e) => this.validConfirmPassword(e.target.value)}/></div>
            </div>
